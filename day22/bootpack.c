...
void HariMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	make_textbox8(sht_win, 8,28,128,16,COL8_FFFFFF); // character heigth is 16pix
	cursor_x = 8;
	...
	*((int *) (console->tss.esp + 8)) = (int) memtotal;
	...
	sheet_updown(sht_mouse,4); // sheet 3 left for test
	...
	for(;;)
	{
		...
		if (i == 256 + 0x46) // ScrollLock 
			key_leds ^= 1;
		if((i == 256 + 0x3b) && (key_shift != 0) && (console->tss.ss0 !=0) )	// shift F1	
		{
			cons = (struct CONSOLE *) *((int *) 0x0fec);
			cons_putstr0(cons, "\n break key : \n");
			io_cli();
			console->tss.eax = (int ) &(console->tss.esp0);
			console->tss.eip = (int ) asm_end_app ;
			io_sti();	
			}
