...
void init_gdtidt(void)
{
	...
	int i;
	// inilialize gdt , 8192 segments 
	for(i=0;i<8192;i++)
		set_segmdesc(gdt+i,0xffffffff,0x00000000,0x4092);

	set_segmdesc(gdt+1,0xffffffff,0x00000000,0x4092); // global 4GB
	set_segmdesc(gdt+2,0x0007ffff,0x00280000,0x409a); // bootpack.hrb 512kb
	load_gdtr(0xffff,0x00270000);  // gdt memory location 270000-27ffff,64kb
