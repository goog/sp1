...
for(i=0;i<MAX_SHEETS;i++)
{
	...
	ctl->sheets0[i].ctl = ctl; // set the ctl pointer
...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) {	
		if (height >= 0) 
			...
			sheet_refreshsub(ctl,sht->vx0,sht->vy0,sht->vx0+sht->bxsize,sht->vy0+sht->bysize,height+1,old);//refresh the bottle sheet 
		    } 
		else {	/* height -1 */
			if (ctl->top > old) {
		...
		// reduce the number of refresh sheets
...
void sheet_refresh( struct SHEET *sht,int bx0,int by0,int bx1,int by1)
{
	...
	if(sht->height >= 0)
	{
		sheet_refreshsub(sht->ctl,sht->vx0+bx0,sht->vy0+by0,sht->vx0+bx1,sht->vy0 + by1,sht->height,sht->height ); // one sheet only
	}
}

// move the sheet position on the background 
void sheet_move(struct SHEET *sht,int vx0,int vy0)
{
int old_vx0 = sht->vx0; // position on the image
int old_vy0 = sht->vy0;
sht->vx0 = vx0;
sht->vy0 = vy0;
if(sht->height >= 0 )
...
struct SHEET * sht;
// refresh area larger than image 
if(vx0 < 0) vx0 = 0;
if(vy0 < 0) vy0 = 0;
if(vx1 > ctl->xsize ) vx1= ctl->xsize;  // image size 
if(vy1 > ctl->ysize ) vy1= ctl->ysize;
// refresh begin at h0 sheet
for(h=h0;h <= h1;h++)
	{
	sht = ctl->sheets[h];
	buf = sht->buf;
	...
	if(by1 > sht->bysize )  by1= sht->bysize; // bx0,by0,bx1,by1 refresh area
	for(by = by0;by < by1;by++)
		{
		vy = sht->vy0+ by;  // related to absoluted path
...
for(h=h0;h <= ctl->top;h++)
	...
	for(by = by0;by < by1;by++)
		...
		{
		vx = sht->vx0+bx;
		c = buf[by * sht->bxsize + bx];  // bx,by  buffer x,y
		if(c != sht->col_inv)
